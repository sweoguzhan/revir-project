...
const EntryFormsTable: FC<{allProjects: AllProject[]}> = ({allProjects}) => {
	...
	const fetchPage = () => {
		...
		setLoading(true);
		getForms(`type=entry&page=${page}${fetchQuery}`)
			.then(response => {
	...
	const changeVerifyStatus = (id: number, status: boolean) => {
		void Swal.fire({
			title: status ? 'Girişi Onayla' : 'Girişi Reddet',
			text: status ? 'Girişi onaylamak istediğinize emin misiniz?' : 'Girişi red etmek istediğinize emin misiniz?',
			...
			if (result.isConfirmed) {
				changeVerifiedStatus({
					...
					if (response.status === 'success') {
						...
						void Swal.fire({
							title: 'Hata Oluştu',
							text: `Giriş onayı değiştirilirken hata oluştu. \n Hata: ${response.message}`,
							icon: 'error',
							confirmButtonText: 'Tamam',
						});
					}
		...
		<>
			<div className='card  '>
				<div className='card-header border-0 pt-4 w-100 align-items-center '>
					<label className=' fw-bold fs-3 mb-2   '>Giriş Formları</label>
					<div className='card-toolbar'>
						<div className='d-flex justify-content-end' >
							<div>
								<button type='button' className='btn btn-primary' data-bs-toggle='modal' data-bs-target='#kt_filter_location_forms_modal'>
